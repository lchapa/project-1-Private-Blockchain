...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
			try{
				...
				block.time = parseInt(Date.now().toString().slice(0,-3));
        ...
        return new Promise(async (resolve, reject) => {
			...
			if ((currentTime - time) < 5*60) {
				if(bitcoinMessage.verify(message, address, signature)) {
					...
					self.validateChain().then(invalidBlocks => {
					...
					reject(Error("Message could not be verified by the chain"));
				}	
			} else {
				reject(Error("Message has expired"));
			}
        });
    }
        ...
        return new Promise((resolve, reject) => {
			self.chain.forEach(b => {
				...
				b.getBData().then(data => {
					//Filter by address from the request
    ...
    validateChain() {
        ...
        return new Promise(async (resolve) => {
			self.chain.forEach(b => {
				b.validate().then((valid) => {
					// Block has been tampered
					if(!valid) {
						errorLog.push(b);
					//Block is valid but chain is broken.
					} else if(b.height > 0) {
